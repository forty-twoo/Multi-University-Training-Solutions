 ...
 * @Don't panic!
 * @Author: forty-twoo
 * @Date: 2019-08-06 12:44
 * @Description: 回文自动机 dfs
 * @Source: https://ac.nowcoder.com/acm/contest/886/C
 */
...
#define N 26
	...
	ll solve() {
		ans = 0;
		mst(nxtTree, 0); mst(failTree, 0);
		dfs(1); dfs(0);
		for (int i = 2; i < p; i++) {
			ans += (ll)failTree[i] * nxtTree[i];
		}
		return ans - (p - 2);
